// based on https://github.com/tokio-rs/tokio/blob/master/tokio-timer/src/wheel/mod.rs

use crate::level::Expiration;
use crate::level::{Level, Entry};
...
impl Wheel {
    ...
    ///           of the timing wheel.
    ///
    /// * `entry`: The `Entry` insert into the wheel.
    ///
    /// # Return
    ///
    /// Returns `Ok` when the id is successfully inserted, `Err` otherwise.
    ///
    ...
    pub fn insert(
        ...
        when: u64,
        entry: Entry,
    ) -> Result<(), (Entry, InsertError)> {
        if when <= self.elapsed {
            return Err((entry, InsertError::Elapsed));
        } else if when - self.elapsed > MAX_DURATION {
            return Err((entry, InsertError::Invalid));
        }

        // Get the level at which the entry should be stored
        let level = self.level_for(when);

        self.levels[level].add_entry(when, entry);
    ...
    /// Remove `Id` from thee timing wheel.
    pub fn remove(&mut self, entry: Entry) {
        let when = entry.when;
        let level = self.level_for(when);

        self.levels[level].remove_entry(when, entry);
    }
    ...
    pub fn poll(&mut self, poll: &mut Poll) -> Option<Entry> {
        loop {
            ...
            match poll.expiration {
                Some(ref expiration) => {
                    if let Some(entry) = self.poll_expiration(expiration) {
                        return Some(entry);
                    }
    ...
    pub fn poll_expiration(
        ...
        while let Some(entry) = self.pop_entry(expiration) {
            if expiration.level == 0 {
                debug_assert_eq!(entry.when, expiration.deadline);
                ...
                self.levels[next_level].add_entry(when, entry);
            }
        }
    ...
    fn pop_entry(&mut self, expiration: &Expiration) -> Option<Entry> {
