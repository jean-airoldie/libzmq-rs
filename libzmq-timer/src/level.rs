// based on https://github.com/tokio-rs/tokio/blob/master/tokio-timer/src/wheel/level.rs
...
/// The size of the stack in the SmallVec.
const STACK_SIZE: usize = 64;

/// A `Slot` contains the entries that are set to expire when the slot
/// reaches level 0.
type Slot = SmallVec<[Entry; 64]>;
...
impl Level {
    ...
    pub fn add_entry(&mut self, when: u64, entry: Entry) {
    ...
    pub fn remove_entry(&mut self, when: u64, entry: Entry) {
        let slot = slot_for(when, self.level);

        if let Some(pos) = self.slots[slot].iter().position(|e| e == &entry) {
    ...
    pub fn pop_entry_slot(&mut self, slot: usize) -> Option<Entry> {
        let ret = self.slots[slot].pop();

        if ret.is_some() && self.slots[slot].is_empty() {
            // The bit is currently set
            debug_assert!(self.occupied & occupied_bit(slot) != 0);
